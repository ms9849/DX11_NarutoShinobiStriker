...
{
	m_pPajama->Set_AnimIndex("CustomMan_Walk_Loop", 0.8f, IsBlend, 0.1f);
}
...
CPajamaState* CPajama_WalkState::Update(_float fTimeDelta)
{
    ...
    if (true == m_pPajama->Use_Kunai() && fDist >= 2.f)
    {
    ...
    else if (fDist >= 2.f)
    {
        ...
        m_pPajama->Get_Transform()->Chase(m_pPlayerTransformCom->Get_State(STATE::POSITION), fTimeDelta * 0.03f, nullptr, fDist);
