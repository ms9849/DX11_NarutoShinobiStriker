...
_bool CPlayer::Use_Skill(SKILLNUM eSlotNum)
{
	...
	if ( pInfo->fTimeAcc >= pInfo->fMaxCoolDown)
	{
		...
		Desc.strParticlePath = TEXT("../Bin/Resources/Particle/UseSkill_Particle.bin");
...
void CPlayer::OnCollision(COLLIDER_HANDLE_ID eHandleID, _float3 vColliderPos)
{
	...
	CParticleObject::PARTICLE_LOAD_DESC Desc;
	Desc.strParticlePath = TEXT("../Bin/Resources/Particle/Player_Hit_Particle.bin");
	XMStoreFloat3(&Desc.vPosition, m_pTransformCom->Get_State(STATE::POSITION));
...
void CPlayer::Set_Gravity(_bool bFlag, _float fDist)
{
	...
	{
		m_pTransformCom->Set_State(STATE::POSITION, m_pTransformCom->Get_State(STATE::POSITION) - XMVectorSet(0.f, 0.1f, 0.f, 0.f));
	}
	else if(fDist > 3.5f && true == m_IsGround)
	{
		CPlayer_FrontJumpState* pNextState = CPlayer_FrontJumpState::Create(this, 1.23f, CPlayer_FrontJumpState::ANIM_STATE::FALL);
		Change_State(pNextState, true);
	}
	else if (fDist < 1.5f && false == m_IsGround)
	{
		CPlayer_LandState* pNextState = CPlayer_LandState::Create(this);
		Change_State(pNextState, true);
	}
}
