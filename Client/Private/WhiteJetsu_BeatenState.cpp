...
CWhiteJetsu_BeatenState::CWhiteJetsu_BeatenState(CNavigation* pNavigation, CWhiteJetsu* pJetsu, _vector vDirection, _float fRatio)
	: m_pJetsu{ pJetsu }
...
void CWhiteJetsu_BeatenState::Start(_bool IsBlend)
{
	...
	if(TEXT("WhiteZetsuCrowdForm_Beaten_Left") != m_pJetsu->Get_CurrentAnim())
		m_pJetsu->Set_AnimIndex("WhiteZetsuCrowdForm_Beaten_Left", 1.5f, IsBlend);
	else
		m_pJetsu->Set_AnimIndex("WhiteZetsuCrowdForm_Beaten_Right", 1.5f, IsBlend);
...
CWhiteJetsuState* CWhiteJetsu_BeatenState::Update(_float fTimeDelta)
{
	...
	if (false == IsAnimFinished && fAnimProgress <= 0.7f)
		m_pJetsu->Get_Transform()->Go_Direction(XMLoadFloat3(&m_vDirection), m_fRatio * fTimeDelta * m_pGameInstance->Calc_Quadratic(-0.89f, 0.48f, 0.11f, fAnimProgress),
			m_pNavigationCom);
...
CWhiteJetsu_BeatenState* CWhiteJetsu_BeatenState::Create(CNavigation* pNavigation, CWhiteJetsu* pJetsu, _vector vDirection, _float fRatio)
{
	return new CWhiteJetsu_BeatenState(pNavigation, pJetsu, vDirection, fRatio);
}
