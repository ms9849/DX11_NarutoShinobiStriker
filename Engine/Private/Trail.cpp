...
void CTrail::Update_Trail(_fmatrix matCurrentWorld, _bool IsMakeTrail)
{
	if (true == IsMakeTrail) {
		...
		XMStoreFloat3(&m_pVTXPOSTEXs[m_iNumPresent + 1].vPosition, XMVector3TransformCoord(XMLoadFloat4(&m_vHigh), matCurrentWorld));
		XMStoreFloat3(&m_pVTXPOSTEXs[m_iNumPresent].vPosition, XMVector3TransformCoord(XMLoadFloat4(&m_vLow), matCurrentWorld));

		m_iNumPresent += 2;
	}
	else if (0 != m_iNumPresent) {
		m_iNumPresent -= 2;
		memmove(m_pVTXPOSTEXs, m_pVTXPOSTEXs + 2, sizeof(VTXPOSTEX) * m_iNumPresent);
	}
